...
class TasksViewsTests(TestCase):
    ...
    def setUp(self):
        ...
        self.task = Task.objects.create(
            name=self.user, taskContent="Test Task", complete=False
        )
    ...
    def test_external_tasks_home_superuser(self):
        ...
        response = self.client.get(
            reverse('External-tasks-home'), {'username': 'testuser'}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Task")
    ...
    def test_filter_completed_tasks(self):
        self.client.login(username='testuser', password='testpass')
        Task.objects.create(
            name=self.user, taskContent="Completed Task", complete=True
        )
    ...
    def test_filter_incomplete_tasks(self):
        self.client.login(username='testuser', password='testpass')
        response = self.client.get(
            reverse('Tasks-home') + '?filter=incomplete'
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Task")
    ...
    def test_filter_last_week_tasks(self):
        ...
        old_task = Task.objects.create(
            name=self.user, taskContent="Old Task", complete=False
        )
        old_task.date_added = now() - timedelta(days=10)
        old_task.save()

        recent_task = Task.objects.create(
            name=self.user, taskContent="Recent Task", complete=False
        )
    ...
    def test_filter_last_month_tasks(self):
        ...
        very_old_task = Task.objects.create(
            name=self.user, taskContent="Very Old Task", complete=False
        )
        very_old_task.date_added = now() - timedelta(days=40)
        very_old_task.save()

        this_month_task = Task.objects.create(
            name=self.user, taskContent="This Month Task", complete=False
        )
